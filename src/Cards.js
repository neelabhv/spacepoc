...
export default function Cards({flight}){
    ...
    return (
        <div id="eachcard">
           <div id="each-flight-detail">Mission : {mission_name} #{flight_number}</div>
           <div id="each-flight-detail">Launch Success : {launch_success ? "True" : "False"}</div>
           <div id="each-flight-detail">Launch Year : {launch_year}</div>
           <div id="each-flight-detail">Launch Site : {site_name}</div>
           <div id="each-flight-detail">Land Success : {land_success ? "True" : "False"}</div>
           <div id="each-flight-detail">Details :  
                {readMore===false ? `${details}`.substring(0,20) : details}
                <button onClick={(e)=>setReadmore(!readMore)}>
            {readMore?"readless":"readmore"}</button>
            </div>
        </div>
    )
}